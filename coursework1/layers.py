...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        epsilon=0.01
        #output = inputs if (np.all(inputs) > 0) else (epsilon * inputs)
        condition1 = (inputs > 0)*inputs    
        condition2 = (inputs <= 0)*(epsilon * inputs)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        #radients = inputs #remove and replace with your code
        epsilon=0.01
        #gradient = 1.0 if (np.all(inputs) > 0) else (epsilon)
        condition1 = (inputs > 0)*1.0
        condition2 = (inputs <= 0)*(epsilon)
        return grads_wrt_outputs* (condition1 + condition2)
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        alpha = 1
        output_condition_1 = (inputs <= 0) * (alpha * (np.exp(inputs) - 1.0))
        output_condition_2 = (inputs > 0) * inputs
        outputs = output_condition_1 + output_condition_2 #remove and replace with your code
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        alpha = 1
        gradient_condition_1 = (inputs <= 0) * (alpha * (np.exp(inputs)))
        gradient_condition_2 = (inputs > 0) * 1.0
        gradients = grads_wrt_outputs * (gradient_condition_1 + gradient_condition_2) #remove and replace with your code
        return gradients
...
class SELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        lamda = 1.0507
        alpha = 1.6733
        output_condition_1 = (inputs <= 0) * (lamda * (alpha * (np.exp(inputs) - 1.0)))
        output_condition_2 = (inputs > 0) * (lamda * inputs)
        outputs = output_condition_1 + output_condition_2 #remove and replace with your code
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        lamda = 1.0507
        alpha = 1.6733
        gradient_condition_1 = (inputs <= 0) * (lamda * (alpha * (np.exp(inputs))))
        gradient_condition_2 = (inputs > 0) * lamda
        gradients = grads_wrt_outputs * (gradient_condition_1 + gradient_condition_2) 
        return gradients
